...
def create_files(files: list[tuple[str, str]], overwrite: bool = False) -> list[dict[str, Any]]:
    ...
    return _to_serializable(build_service().create_files(files=files, overwrite=overwrite))
...
def read_files(
    ...
    return _to_serializable(build_service().read_files(paths=paths, line_intervals=line_intervals))
...
def copy_or_move_files(paths: list[str], destination: str, move: bool = False) -> list[dict[str, Any]]:
    ...
    return _to_serializable(build_service().copy_or_move_files(paths=paths, destination=destination, move=move))


@mcp.tool
def create_directories(paths: list[str], exist_ok: bool = True) -> list[dict[str, Any]]:
    """Create one or more directories."""
    return _to_serializable(build_service().create_directories(paths=paths, exist_ok=exist_ok))
...
def update_directories(paths: list[str], destination: str, move: bool = True) -> list[dict[str, Any]]:
    ...
    return _to_serializable(build_service().update_directories(sources=paths, destination=destination, move=move))


@mcp.tool
def delete_directories(paths: list[str], recursive: bool = True) -> list[dict[str, Any]]:
    """Delete one or more directories."""
    return _to_serializable(build_service().delete_directories(paths=paths, recursive=recursive))


@mcp.tool
def search_directories(base_path: str, name_pattern: str | None = None) -> list[str]:
    """Search directories by name pattern."""
    return build_service().search_directories(base_path=base_path, name_pattern=name_pattern)
...
def copy_or_move_directories(paths: list[str], destination: str, move: bool = False) -> list[dict[str, Any]]:
    ...
    return _to_serializable(build_service().copy_or_move_directories(paths=paths, destination=destination, move=move))


@mcp.tool
def filesystem_outline(base_path: str, depth: int = 3) -> dict[str, Any]:
    """Build a hierarchical outline of the filesystem."""
    return _to_serializable(build_service().filesystem_outline(base_path=base_path, depth=depth))
